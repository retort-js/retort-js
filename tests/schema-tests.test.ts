...
type MongooseSchemaPrimitiveType = 
  | StringConstructor
  | NumberConstructor
  | BooleanConstructor
  | DateConstructor
...
type MongooseSchemaType = MongooseSchemaPrimitiveType | MongooseSchemaDefinition | [MongooseSchemaType];

/**
 * @typedef {Object} MongooseSchemaDefinition
 * @property {MongooseSchemaType} type - The type of the schema.
 * @property {boolean} [optional] - Indicates if the field is optional.
 * @property {string[]} [options] - An array of possible values.
 * @property {string} [description] - A description of the field.
 * @property {boolean} [nullable] - Indicates if the field can be null.
 * @property {number} [minLength] - The minimum length of a string.
 * @property {number} [maxLength] - The maximum length of a string.
 * @property {string} [pattern] - The pattern of a string. deprecated Use `regEx` instead.
 * @property {string} [format] - The format of the field.
 * @property {number} [minimum] - The minimum value of a number.
 * @property {number} [maximum] - The maximum value of a number.
 * @property {number} [exclusiveMinimum] - The exclusive minimum value of a number.
 * @property {number} [exclusiveMaximum] - The exclusive maximum value of a number.
 * @property {number} [multipleOf] - The number must be a multiple of this value.
 * @property {any} [const] - A constant value. deprecated Use `value` instead.
 * @property {any} [value] - A constant value.
 * @property {string} [regEx] - The regular expression pattern of a string.
 * @property {string[]} [enum] - An array of possible values. deprecated Use `options` instead.
 */
...
function mongooseTypeToJsonSchemaType(type: MongooseSchemaPrimitiveType | MongooseSchemaDefinition | [MongooseSchemaType]): string {
  switch (type) {
    case String:
      return 'string';
    case Number:
...
function convertMongooseToJsonSchema(mongooseSchema: MongooseSchemaDefinition): JsonSchema {
  ...
  for (const [key, value] of Object.entries(mongooseSchema)) {
    ...
    if (typeof value === 'object' && !Array.isArray(value) && 'type' in value) {
      ...
      if (schemaTypeOpts.options && schemaTypeOpts.enum) {
        throw new Error(`Both 'options' and 'enum' are specified for ${key}`);
      }
      if (schemaTypeOpts.options) {
        propertySchema.enum = schemaTypeOpts.options;
      }
      ...
      if (schemaTypeOpts.pattern && schemaTypeOpts.regEx) {
        throw new Error(`Both 'pattern' and 'regEx' are specified for ${key}`);
      }
      if (schemaTypeOpts.pattern) {
        propertySchema.pattern = schemaTypeOpts.pattern;
      }
      if (schemaTypeOpts.regEx) {
        propertySchema.pattern = schemaTypeOpts.regEx;
      }
...
describe('convertMongooseToJsonSchema', () => {
  ...
  it('should handle "options" property', () => {
    const mongooseSchema: MongooseSchemaDefinition = {
      status: { type: String, options: ['active', 'inactive'], optional: false },
    };
  ...
  it('should handle "regEx" property', () => {
    const mongooseSchema: MongooseSchemaDefinition = {
      username: { type: String, regEx: '^[a-zA-Z0-9_]+$', optional: false },
    };
  ...
  it('should throw error if both "enum" and "options" are specified', () => {
    const mongooseSchema: MongooseSchemaDefinition = {
      status: { type: String, enum: ['active'], options: ['inactive'], optional: false },
    };

    expect(() => convertMongooseToJsonSchema(mongooseSchema)).toThrow(Error);
  });

  it('should throw error if both "pattern" and "regEx" are specified', () => {
    const mongooseSchema: MongooseSchemaDefinition = {
      username: { type: String, pattern: '^[a-zA-Z0-9_]+$', regEx: '^[a-zA-Z0-9_]+$', optional: false },
    };
